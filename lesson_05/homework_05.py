...
# 1
people_records.insert(0,('John', 'Rembo', 35, 'Engineer', 'Lviv'))
...
print(people_records)
print("-" * 100)
# 3 (як я зрозумів потрібно перевірити чи всі люди мають вік ≥ 30 одночасно, тому зробив якщо хоч один немає то False)
indexes_people_records= [6, 10, 13]
selected_records = [people_records[i] for i in indexes_people_records]
print(f'Генерація вихідного списку: {selected_records}')
condition = True
for record in selected_records:
    if record[2] < 30:  #якщо хоча б один вік <30
        condition = False
        break
print(condition)
...
#Пошук у вигляді кортежу (рік ≥, об'єм двигуна ≥, ціна ≤)
year, engine, price = search_criteria
found_cars = [(brand, info)
    for brand, info in car_data.items()
    if info[1] >= year and info[2] >= engine and info[4] <= price]
print(found_cars)
print("-" * 100)
#Автомобілі повинні бути відсортовані за зростанням ціни
#x - кортеж (brand, info)
#x[1] - доступ до кортежа info
#[1][4] - ціна авто
found_cars.sort(key=lambda x: x[1][4])
print(found_cars)
print("-" * 100)
#Ми повинні вивести до п'яти (5) перших знайдених елементів
for brand, info in found_cars[:5]:
    print(f"{brand}: {info}")
